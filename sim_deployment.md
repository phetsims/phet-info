...
3. The following steps assume `grunt` has been run.  If the version of chipper used by this simulation is from after
November 10, 2015, run `grunt deploy-production`. If the chipper version is from prior to November 10, 2015, run  `grunt
deploy-production --locales=*`.  Either of these commands will deploy to spot, and then use the build server to deploy
to production. (Note: In order to determine the commit date for the chipper SHA, grab the SHA out of the 
dependencies.json file, and from a gitbash command line enter `git log --pretty=format:"%H %cd" | grep <SHA>`.)
